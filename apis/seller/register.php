...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($seller->validate_params($_POST['gender'])) {
        $seller->gender = $_POST['gender'];
    } else {
    ...
    if ($seller->check_unique_email()) {
         if (($id = $seller->register_seller()) !=false) {            echo json_encode(array('success' => 1, 'message' => 'Seller regstered!' , 'id' => (int)$id['id']));
